...
import { Ad } from '@/pages/Ads/columns';
import { Device } from '@/pages/Devices/columns';
        ...
        .map((event) => {
          ...
          return (
            <div
              key={event.id}
              className={cn(
                "relative p-4 rounded-lg shadow-md text-white",
            ...
            >
              <div className="flex flex-col space-y-2">
                ...
                {/* <div className="text-sm opacity-80">
                  ...
                  <Eye className="w-5 h-5 cursor-pointer" onClick={()=>navigate(`/ads/${event.ad.ad_id}`)} />
                  </p>
                  <p>
                    Ad ID:{" "}
                    <span className="font-medium">{event.ad.ad_id}</span>
                  </p>
                  <div className="">
                    AD File:{" "}
                  <a
                ...
                <div className="text-sm opacity-80">
                  <p className='flex flex-row space-x-2'>
                    Device:{" "}
                    <span className="font-medium">{event.device.location}</span>
                  <Eye className="w-5 h-5 cursor-pointer" onClick={()=>navigate(`/device/${event.device.device_id}`)} />
                  </p>
                  <p>
                    Device ID:{" "}
                    <span className="font-medium">{event.device.device_id}</span>
                  </p>
                  <p>
                    Status:{" "}
                    <span
                      className={`font-medium ${
                        event.device.status === "active" ? "text-green-500" : "text-red-500"
                      }`}
                    >
                      {event.device.status}
                    </span>
