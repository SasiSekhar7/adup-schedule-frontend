...
export function NotificationBell() {
  ...
  // Fetch notifications ONLY from backend database API
  const fetchNotifications = async () => {
    try {
      setIsLoading(true);
      const res = await axios.get(`${BACKEND_URL}/notifications/list`, {
        params: { limit: 30 },
      });
  ...
  const handleMarkAllAsRead = async () => {
    try {
      ...
      setUnreadCount(0);
      await axios.post(`${BACKEND_URL}/notifications/mark-all-read`, {});
    } catch (err) {
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-muted/30">
          <div className="flex items-center space-x-2">
            ...
            {unreadCount > 0 && (
              <Badge variant="secondary" className="text-[11px] px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 font-medium">
                {unreadCount} new
              </Badge>
            )}
          </div>
        ...
        <div className="max-h-[360px] overflow-y-auto divide-y divide-border">
          {filteredNotifications.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
              ...
              <p className="text-sm font-medium text-foreground mb-1">No notifications found</p>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-1 mb-1">
                    <p className={`text-xs font-semibold truncate ${!item.is_read ? "text-foreground font-bold" : "text-muted-foreground"}`}>
                      {item.title}
                    </p>
                ...
                <div className="flex items-center space-x-1 shrink-0">
                  {!item.is_read && (
                    <span className="w-2 h-2 rounded-full bg-blue-600 my-auto" title="Unread"></span>
                  )}
                  <Button
                    variant="ghost"
                    size="icon"
