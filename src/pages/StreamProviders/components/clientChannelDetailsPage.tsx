...
export default function ClientChannelDetails() {
  const { slug: providerSlug } = useParams();
  ...
  const startWebcamLive = async () => {
    ...
    try {
      ...
      if (channel.status !== "live") {
        await api.put(`/streaming/channel/${channel.channel_id}/start`);
        await fetchChannelById(); // refresh state
      }
      ...
      recorder.ondataavailable = async (event) => {
        if (event.data.size > 0) {
          await fetch(
            `https://stg-cms.ad96.in/api/stream/${channel.channel_id}`,
            {
              method: "POST",
              headers: {
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      ...
      navigator.sendBeacon(
        "/api/stop-stream",
        JSON.stringify({ channel_id: channel.channel_id }),
      );
    };
