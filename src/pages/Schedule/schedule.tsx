...
function Schedule() {
  ...
  useEffect(() => {
    async function fetchSchedules() {
      try {
        ...
        const transformedEvents = await response.events.map((event) => ({
          id: event.id,
          ...
          ad: event.ad,
          device: event.device,
          color: "blue", // Assume that you also have color field
        }));
  ...
  return (
    <div className="max-h-[80vh] h-[70vh]">
      <div className="flex items-center w-full mb-4">
        <div className="">
          <p className="text-md font-semibold ">Ad Schedule</p>
          <p className="text-sm text-muted-foreground">All Scheduled ads</p>
        </div>
