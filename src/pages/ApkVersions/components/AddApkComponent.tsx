...
function AddApkComponent({ onApkAdded }: { onApkAdded: () => void }) {
  ...
  const [estimatedFileSizeBytes, setEstimatedFileSizeBytes] = useState<
    number | undefined
  >(undefined);
  ...
  useEffect(() => {
    const fetchLatestVersion = async () => {
      try {
        ...
        }>("/apk_versions/latest"); // Assuming this API endpoint
        ...
        console.error(
          "Failed to fetch latest APK version for suggestion:",
          err,
        );
  ...
  const handleCreateApk = async () => {
    ...
    try {
      ...
      if (
        !apkData.version_code ||
        apkData.version_name === "" ||
        apkData.file_name === ""
      ) {
        setError(
          "Please fill in all required APK details (Version Code, Version Name, File Name).",
        );
        toast.error("Validation Error", {
          description: "Please fill in all required APK details.",
        });
        return; // Exit early
      }
      if (isNaN(apkData.version_code) || apkData.version_code <= 0) {
        ...
        toast.error("Validation Error", {
          description: "Version Code must be a positive integer.",
        });
        return; // Exit early
      }
      ...
      await toast.promise(
        api.post<ApkVersion>("/apk_versions", formData, {
          headers: {
        ...
        }),
        {
          ...
          success: (response) => {
            // 'response' here is the result of the promise (the axios response object)
          ...
          error: (err) => {
            ...
            const errorMessage =
              ...
              "An unexpected error occurred while adding APK.";
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input
              ...
              onChange={(e) =>
                setApkData((prev) => ({
                  ...prev,
                  version_code: parseInt(e.target.value) || 0,
                }))
              }
              className="col-span-3"
              required
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input
              ...
              onChange={(e) =>
                setApkData((prev) => ({ ...prev, file_name: e.target.value }))
              }
              className="col-span-3"
              required
            />
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Checkbox
              ...
              onCheckedChange={(checked: boolean) =>
                setApkData((prev) => ({ ...prev, is_mandatory: checked }))
              }
              className="col-span-3"
            />
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Checkbox
              ...
              onCheckedChange={(checked: boolean) =>
                setApkData((prev) => ({ ...prev, is_active: checked }))
              }
              className="col-span-3"
            />
          </div>
          ...
          {error && (
            <span className="text-red-500 text-sm col-span-full text-center">
              {error}
            </span>
          )}
        </div>
